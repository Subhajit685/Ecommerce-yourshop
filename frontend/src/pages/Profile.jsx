    ...
    <aside className={`bg-white min-h-[95vh] w-full md:max-w-60 max-w-24 shadow-md transition-all fixed z-50 ${open ? "block" : "hidden"} transition-all`}>
        ...
        <div className='my-5 md:my-10'>
            <nav className='grid gap-1 md:mx-8 mx-2'>
                <Link to={"/myorder"} className='hover:bg-orange-300 transition-all p-2 rounded-sm text-xs md:text-lg'>My Order</Link>
                <Link to={"/user-profile"} className='hover:bg-orange-300 transition-all p-2 rounded-sm text-xs md:text-lg'>My Profile</Link>
            </nav>
        </div>
    </aside>
